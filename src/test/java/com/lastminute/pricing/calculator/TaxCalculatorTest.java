...
public class TaxCalculatorTest extends TaxCalculator {
	...
	public void testRun() {
		...
		CommerceItemVO ci1 = new CommerceItemVO("ciId", sku1, 2, 1.00, 0, 2.00, 0);
		
		OrderVO order = new OrderVO("orderId", Lists.newArrayList(ci,ci1), 0.0, 0.0, 0.0);
		
		order = run(order);
		assertTrue(ci.getTaxAmount() == 0);
		assertTrue(ci1.getTaxAmount() == 0.2);
		
		assertTrue(ci.getTotalPrice() == 11.0);
		assertTrue(ci1.getTotalPrice() == 2.2);
